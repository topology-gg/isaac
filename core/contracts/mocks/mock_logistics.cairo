...
func mock_harvester_resource_concentration_to_quantity_per_tick{}(
    ...
    let (
        base_quantity
    ) = ns_logistics_harvester.harvester_resource_concentration_to_quantity_per_tick(
...
@view
func mock_harvester_energy_to_boost_factor{}(resource_type: felt, energy: felt) -> (
    boost_factor: felt
) {
...
@view
func mock_transformer_resource_type_to_base_quantity_per_tick{}(resource_type_from: felt) -> (
    base_quantity_to: felt
) {
    let (
        base_quantity_to
    ) = ns_logistics_transformer.transformer_resource_type_to_base_quantity_per_tick(
        resource_type_from
    );
...
@view
func mock_transformer_energy_to_boost_factor{}(resource_type: felt, energy: felt) -> (
    boost_factor: felt
) {
...
@view
func mock_transformer_quantity_per_tick{range_check_ptr}(resource_type: felt, energy: felt) -> (
    quantity_to: felt
) {
...
@view
func mock_utb_quantity_should_send_per_tick{range_check_ptr}(quantity_source) -> (
...
@view
func mock_utb_set_length_to_decay_factor{range_check_ptr}(length: felt) -> (decay_factor: felt) {
    let (decay_factor) = ns_logistics_utb.utb_set_length_to_decay_factor(length);
...
func mock_utb_quantity_should_receive_per_tick{range_check_ptr}(
    quantity_source: felt, length: felt
...
@view
func mock_utl_energy_should_send_per_tick{range_check_ptr}(energy_source: felt) -> (
    energy_should_send: felt
) {
    let (energy_should_send) = ns_logistics_utl.utl_energy_should_send_per_tick(energy_source);

    return (energy_should_send,);
}

@view
func mock_utl_set_length_to_decay_factor{range_check_ptr}(length: felt) -> (decay_factor: felt) {
    let (decay_factor) = ns_logistics_utl.utl_set_length_to_decay_factor(length);
