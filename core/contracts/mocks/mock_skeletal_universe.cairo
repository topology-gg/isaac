...
@external
func test_terminate_universe_and_notify_lobby{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    ...
    recurse_prepare_play_record(bool_universe_escape_condition_met, arr_play, 0);
...
func recurse_prepare_play_record{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    bool_universe_escape_condition_met: felt, arr_play: Play*, idx: felt
) -> () {
    alloc_locals;
    ...
    // Tail recursion
    //
    recurse_prepare_play_record(bool_universe_escape_condition_met, arr_play, idx + 1);
    return ();
}

@external
func test_write_civilization_player_address_to_has_launched_ndpe{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(player_adr: felt, has_launched_ndpe: felt) -> () {
    ns_universe_state_functions.civilization_player_address_to_has_launched_ndpe_write(
        player_adr, has_launched_ndpe
    );
