...
from contracts.util.dynamics_ops import (
    dynamics_add,
    dynamics_mul_scalar_fp,
    dynamics_mul_scalar,
    dynamics_div_scalar,
)
...
//
@event
func forward_world_macro_occurred(event_counter: felt, macro_state: Dynamics, phi: felt) {
}

@event
func impulse_applied_occurred(impulse: Vec2) {
}
...
//
func rk4{syscall_ptr: felt*, range_check_ptr}(dt: felt, state: Dynamics) -> (state_nxt: Dynamics) {
    alloc_locals;
...
//
func differentiate{syscall_ptr: felt*, range_check_ptr}(state: Dynamics) -> (state_diff: Dynamics) {
    alloc_locals;
...
//
func forward_planet_spin{range_check_ptr}(phi) -> (phi_nxt: felt) {
...
func forward_planet_dynamic_applying_impulse{range_check_ptr}(
    dynamic_pre_impulse: Dynamic, impulse_fp: Vec2
...
//
func forward_planet_dynamic_applying_perturbation{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(dynamic_pre_perturbation: Dynamic) -> (dynamic_post_perturbation: Dynamic) {
    alloc_locals;
    ...
    //
    let (entropy) = hash2{hash_ptr=pedersen_ptr}(
        dynamic_pre_perturbation.qd.x, dynamic_pre_perturbation.qd.y
    );
    let (prn) = ns_prng.get_prn_mod(mod=2001, entropy=entropy);  // range: [0, 2000]
    let prn_shifted = prn - 1000;  // range: [-1000, 1000]
    let (rotation, _) = signed_div_rem(
        ns_perturb.ROTATION_BOUND * prn_shifted, 1000, RANGE_CHECK_BOUND
    );

    //
    // Apply rotation to perturbation vector
    //
    let (perturb_vec_after_rotation) = compute_vector_rotate(perturb_vec_before_rotation, rotation);
...
func forward_world_macro{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> () {
    ...
    let (plnt_dynamic_post_impulse) = forward_planet_dynamic_applying_impulse(
        state_curr.plnt, impulse_aggregated
    );
    ...
    //
    let (state_nxt: Dynamics) = rk4(dt=DT, state=state_curr_perturbed);
    let (phi_nxt) = forward_planet_spin(phi=phi_curr);
    ...
    forward_world_macro_occurred.emit(
        event_counter=event_counter, macro_state=state_nxt, phi=phi_nxt
    );
...
//
func is_world_macro_escape_condition_met{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() -> (met: felt) {
    alloc_locals;
