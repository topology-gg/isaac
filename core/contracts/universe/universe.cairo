...
from contracts.util.structs import Vec2, Dynamic, Dynamics, Play
...
//
@event
func activate_universe_occurred(event_counter: felt, civ_idx: felt) {
}

@event
func give_undeployed_fungible_device_occurred(
    event_counter: felt, to: felt, type: felt, amount: felt
) {
}

@event
func player_deploy_device_occurred(event_counter: felt, owner: felt, device_id: felt, grid: Vec2) {
}

@event
func player_pickup_device_occurred(event_counter: felt, owner: felt, device_id: felt, grid: Vec2) {
}
...
@event
func player_pickup_utx_occurred(event_counter: felt, owner: felt, grid: Vec2) {
}

@event
func player_upsf_build_fungible_device_occurred(
    event_counter: felt, owner: felt, grid: Vec2, device_type: felt, device_count: felt
) {
}

@event
func player_transfer_undeployed_fungible_device_occurred(
    event_counter: felt, src: felt, dst: felt, device_type: felt, device_amount: felt
) {
}

@event
func player_transfer_undeployed_nonfungible_device_occurred(
    event_counter: felt, src: felt, dst: felt, device_id: felt
) {
}
...
namespace IContractLobby {
    func universe_report_play(arr_play_len: felt, arr_play: Play*) -> () {
    }
}
...
func assert_address_in_civilization{
...
func recurse_reset_civilization_registry{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(idx: felt) -> () {
    if (idx == CIV_SIZE) {
        return ();
...
func activate_universe{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    arr_player_adr_len: felt, arr_player_adr: felt*
) -> () {
    alloc_locals;
    ...
    //
    recurse_populate_civilization_player_states(arr_player_adr, 0);
    ...
    activate_universe_occurred.emit(event_counter, new_civ_idx);

    return ();
}

func recurse_populate_civilization_player_states{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(arr_player_adr: felt*, idx: felt) -> () {
    alloc_locals;
    ...
    // Tail recursion
    //
    recurse_populate_civilization_player_states(arr_player_adr, idx + 1);
    return ();
}
...
func anyone_forward_universe{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (
    ...
    with_attr error_message(
            "last-update block must be at least {min_dist} block away from current block.") {
        assert bool = 1;
    }
    ...
    if (bool_universe_terminable == 1) {
        ...
        terminate_universe_and_notify_lobby(
            destruction_by_which_sun, bool_universe_escape_condition_met
        );
...
func terminate_universe_and_notify_lobby{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(destruction_by_which_sun: felt, bool_universe_escape_condition_met: felt) -> () {
    alloc_locals;
    ...
    recurse_prepare_play_record(
        destruction_by_which_sun, bool_universe_escape_condition_met, arr_play, 0
    );
    IContractLobby.universe_report_play(lobby_address, CIV_SIZE, arr_play);
...
func recurse_prepare_play_record{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (
        has_launched_ndpe
    ) = ns_universe_state_functions.civilization_player_address_to_has_launched_ndpe_read(
        player_address
    );
    ...
    //
    recurse_prepare_play_record(
        destruction_by_which_sun, bool_universe_escape_condition_met, arr_play, idx + 1
    );
    return ();
}
...
func player_deploy_device{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (event_counter) = ns_universe_state_functions.event_counter_read();
    ns_universe_state_functions.event_counter_increment();
    player_deploy_device_occurred.emit(event_counter, caller, device_id, grid);
...
func player_pickup_device_by_grid{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (event_counter) = ns_universe_state_functions.event_counter_read();
    ns_universe_state_functions.event_counter_increment();
    player_pickup_device_occurred.emit(event_counter, caller, device_id, grid);
...
func player_deploy_utx_by_grids{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    utx_device_type: felt, src_device_grid: Vec2, dst_device_grid: Vec2, locs_len: felt, locs: Vec2*
) -> () {
    alloc_locals;
    ...
    let (utx_label) = ns_micro_utx.utx_deploy(
        caller, utx_device_type, locs_len, locs, src_device_grid, dst_device_grid
    );
...
func player_pickup_utx_by_grid{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (event_counter) = ns_universe_state_functions.event_counter_read();
    ns_universe_state_functions.event_counter_increment();
    player_pickup_utx_occurred.emit(event_counter=event_counter, owner=caller, grid=grid);

    return ();
}

@external
func player_upsf_build_fungible_device{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(grid: Vec2, device_type: felt, device_count: felt) -> () {
    alloc_locals;

    let (caller) = get_caller_address();
    assert_address_in_civilization(caller);

    ns_micro_devices.upsf_build_fungible_device(caller, grid, device_type, device_count);

    let (event_counter) = ns_universe_state_functions.event_counter_read();
    ns_universe_state_functions.event_counter_increment();
    player_upsf_build_fungible_device_occurred.emit(
        event_counter, caller, grid, device_type, device_count
    );
...
@external
func player_upsf_build_nonfungible_device{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(grid: Vec2, device_type: felt, device_count: felt) -> () {
    let (caller) = get_caller_address();
    assert_address_in_civilization(caller);

    ns_micro_devices.upsf_build_nonfungible_device(caller, grid, device_type, device_count);
...
@external
func player_launch_all_deployed_ndpe{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(grid: Vec2) -> () {
    ...
    //
    ns_micro_devices.launch_all_deployed_ndpe(caller, grid);

    return ();
}

@external
func player_transfer_undeployed_fungible_device{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(type: felt, amount: felt, to_player_idx: felt) -> () {
    //
    ...
    //
    let (from_curr_amount) = ns_micro_state_functions.fungible_device_undeployed_ledger_read(
        caller, type
    );
    ...
    // Make transfer
    //
    let (to_curr_amount) = ns_micro_state_functions.fungible_device_undeployed_ledger_read(
        to, type
    );
    ns_micro_state_functions.fungible_device_undeployed_ledger_write(
        caller, type, from_curr_amount - amount
    );
    ns_micro_state_functions.fungible_device_undeployed_ledger_write(
        to, type, to_curr_amount + amount
    );
    ...
    player_transfer_undeployed_fungible_device_occurred.emit(
        event_counter=event_counter, src=caller, dst=to, device_type=type, device_amount=amount
    );
...
@external
func player_transfer_undeployed_nonfungible_device{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(device_id: felt, to_player_idx: felt) -> () {
    //
    // get player address at to_player_idx
    //
    let (to) = ns_universe_state_functions.civilization_player_idx_to_address_read(to_player_idx);
    ...
    with_attr error_message(
            "The device with this device_id is deployed, hence unable to transfer") {
    ...
    player_transfer_undeployed_nonfungible_device_occurred.emit(
        event_counter=event_counter, src=caller, dst=to, device_id=device_id
    );
...
func anyone_view_all_utx_grids{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    utx_device_type: felt
) -> (grids_len: felt, grids: Vec2*) {
    let (grids_len, grids) = ns_micro_iterator.iterate_utx_deployed_emap_grab_all_utxs(
        utx_device_type
    );
...
func give_undeployed_device{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    if (bool_is_utx == 1) {
        ...
        let (event_counter) = ns_universe_state_functions.event_counter_read();
        ns_universe_state_functions.event_counter_increment();
        give_undeployed_fungible_device_occurred.emit(event_counter, to, type, amount);
        tempvar syscall_ptr = syscall_ptr;
        tempvar pedersen_ptr = pedersen_ptr;
        tempvar range_check_ptr = range_check_ptr;
    } else {
        ...
        recurse_give_undeployed_device(
            idx=0, len=amount, block_height=block_height, to=to, type=type
        );
        tempvar syscall_ptr = syscall_ptr;
        tempvar pedersen_ptr = pedersen_ptr;
        tempvar range_check_ptr = range_check_ptr;
...
func recurse_give_undeployed_device{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(idx: felt, len: felt, block_height: felt, to: felt, type: felt) {
    alloc_locals;
    ...
    ns_micro_devices.create_new_nonfungible_device(
        block_height=block_height, owner=to, device_type=type
    );

    //
    // Tail recursion
    //
    recurse_give_undeployed_device(idx + 1, len, block_height, to, type);
