...
from contracts.design.constants import PLANET_DIM
...
//
func is_valid_grid{range_check_ptr}(grid: Vec2) -> () {
    alloc_locals;
...
//
func are_contiguous_grids_given_valid_grids{range_check_ptr}(grid0: Vec2, grid1: Vec2) -> () {
    alloc_locals;
    ...
    with_attr error_message(
            "two grids on different faces need to be on same edge-number to be possibly contiguous") {
...
func locate_face_and_edge_given_valid_grid{range_check_ptr}(grid: Vec2) -> (
    face: felt, is_on_edge: felt, edge: felt, idx_on_edge: felt
) {
    alloc_locals;
...
func is_zero{range_check_ptr}(x) -> (bool: felt) {
