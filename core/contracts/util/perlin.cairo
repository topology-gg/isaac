...
func normalize_grid{range_check_ptr}(face: felt, grid: Vec2) -> (pos: Vec2) {
    if (face == 0) {
...
func get_random_vecs{range_check_ptr}(face: felt, element_type: felt) -> (
    rv_bottom_left: Vec2, rv_top_left: Vec2, rv_bottom_right: Vec2, rv_top_right: Vec2
) {
    alloc_locals;
    ...
    if (face == 0) {
        ...
        return (rv_bottom_left, rv_top_left, rv_bottom_right, rv_top_right);
    }
...
func dot{range_check_ptr}(v1: Vec2, v2: Vec2) -> (res: felt) {
    return (v1.x * v2.x + v1.y * v2.y,);
}

func fade{range_check_ptr}(t: felt) -> (res: felt) {
    //
...
func lerp{range_check_ptr}(t: felt, a: felt, b: felt) -> (res: felt) {
    //
...
func get_perlin_value{syscall_ptr: felt*, range_check_ptr}(
    ...
    let pv_bottom_left_fp = Vec2(pos.x * S, pos.y * S);
    let pv_top_left_fp = Vec2(pos.x * S, (pos.y - (PLANET_DIM - 1)) * S);
    let pv_bottom_right_fp = Vec2((pos.x - (PLANET_DIM - 1)) * S, pos.y * S);
    let pv_top_right_fp = Vec2((pos.x - (PLANET_DIM - 1)) * S, (pos.y - (PLANET_DIM - 1)) * S);
