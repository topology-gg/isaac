...
from contracts.design.constants import (
    ...
    ns_device_types,
    ns_element_types,
    ns_base_harvester_multiplier,
    ns_harvester_boost_factor,
    ns_base_transformer_quantity,
    ns_transformer_boost_factor,
    ns_solar_power,
    ns_nuclear_power,
    ns_decay_function,
    ns_perlin,
...
namespace ns_logistics_harvester {
    func harvester_resource_concentration_to_quantity_per_tick{}(
        resource_type: felt, concentration: felt
    ) -> (base_quantity: felt) {
    ...
    func harvester_energy_to_boost_factor{}(resource_type: felt, energy: felt) -> (
        boost_factor: felt
    ) {
    ...
    func harvester_quantity_per_tick{range_check_ptr}(
        resource_type: felt, concentration: felt, energy: felt
    ) -> (quantity: felt) {
        alloc_locals;

        let (f) = harvester_resource_concentration_to_quantity_per_tick(
...
namespace ns_logistics_transformer {
    func transformer_resource_type_to_base_quantity_per_tick{}(resource_type_from: felt) -> (
    ...
    func transformer_energy_to_boost_factor{}(resource_type: felt, energy: felt) -> (
        boost_factor: felt
    ) {
    ...
    func transformer_quantity_per_tick{range_check_ptr}(resource_type: felt, energy: felt) -> (
        quantity_to: felt
    ) {
        alloc_locals;
...
namespace ns_logistics_xpg {
    func spg_solar_exposure_fp_to_energy_generated_per_tick{range_check_ptr}(
        solar_exposure_fp: felt
    ) -> (energy_generated: felt) {
        let (energy_generated, _) = unsigned_div_rem(
            solar_exposure_fp * ns_solar_power.MULT, ns_solar_power.BOUND * SCALE_FP
        );
...
namespace ns_logistics_utb {
    func utb_quantity_should_send_per_tick{range_check_ptr}(quantity_source) -> (
    ...
    func utb_set_length_to_decay_factor{range_check_ptr}(length: felt) -> (decay_factor: felt) {
        //
    ...
    func utb_quantity_should_receive_per_tick{range_check_ptr}(
        quantity_should_send: felt, length: felt
        ...
        let (quantity_should_receive, _) = unsigned_div_rem(
            quantity_should_send * decay_factor, denominator
        );
...
namespace ns_logistics_utl {
    func utl_energy_should_send_per_tick{range_check_ptr}(energy_source: felt) -> (
    ...
    func utl_set_length_to_decay_factor{range_check_ptr}(length: felt) -> (decay_factor: felt) {
        //
        // e^(-x), where x = lambda * length
        // via taylor expansion to the third degree:
        // e^(-x) ~= 1 - x + x^2/2 - x^3/6
    ...
    func utl_energy_should_receive_per_tick{range_check_ptr}(
        energy_should_send: felt, length: felt
        ...
        let (energy_should_receive, _) = unsigned_div_rem(
            energy_should_send * decay_factor, denominator
        );
