...
from contracts.util.perlin import get_perlin_value
...
namespace ns_distribution {
    @view
    func get_concentration_at_grid_given_element_type{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(grid: Vec2, element_type: felt) -> (res: felt) {
...
func get_adjusted_perlin_value{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    //
    let (value_fp) = get_perlin_value(face, grid, element_type);

    //
    // Adjust value for given scaler and offset derived from `element_type`
    //
    let (res) = adjust(value_fp, offset);

    return (res,);
}

func adjust{range_check_ptr}(x_fp: felt, offset: felt) -> (res: felt) {
    //
    // adjust = lambda x : math.floor ( (x + offset)**2 )
    //
    let (res_fp) = mul_fp(x_fp + offset * SCALE_FP, x_fp + offset * SCALE_FP);
