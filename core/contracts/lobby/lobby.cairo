...
from contracts.util.structs import Play
from contracts.design.constants import GYOZA, CIV_SIZE, UNIVERSE_COUNT
from contracts.lobby.lobby_state import ns_lobby_state_functions
from contracts.lobby.ticket_state import ns_ticket_state
...
@event
func ask_to_queue_occurred(event_counter: felt, account: felt, queue_idx: felt) {
}

@event
func give_invitation_occurred(event_counter: felt, account: felt) {
}
...
namespace IContractUniverse {
    func activate_universe(arr_player_adr_len: felt, arr_player_adr: felt*) -> () {
    }

    func check_address_in_civilization(address: felt) -> (bool: felt) {
    }
}

@contract_interface
namespace IContractDAO {
    func subject_report_play(arr_play_len: felt, arr_play: Play*) -> () {
    }
}
...
namespace IContractS2m2 {
    func read_s2m_solver_record(address: felt) -> (record: Record) {
    }
}
...
//
@view
func probe_can_dispatch_to_universe{
...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    ...
    give_invitation_occurred.emit(event_counter, GYOZA);
...
func init_give_invitations_once{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    arr_guests_len: felt, arr_guests: felt*
) {
...
func recurse_give_invitations{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    idx: felt, arr_addr_len: felt, arr_addr: felt*
) -> () {
    if (idx == arr_addr_len) {
        return ();
    }
    ...
    let (event_counter) = ns_lobby_state_functions.event_counter_read();
    ns_lobby_state_functions.event_counter_increment();
    give_invitation_occurred.emit(event_counter, arr_addr[idx]);

    recurse_give_invitations(idx + 1, arr_addr_len, arr_addr);
    return ();
}

@external
func set_universe_addresses_once{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    universe_addresses_len: felt, universe_addresses: felt*
) {
    //
    ...
    recurse_write_universe_addresses(universe_addresses_len, universe_addresses, 0);

    return ();
}

func recurse_write_universe_addresses{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(adr_len: felt, adr: felt*, idx: felt) -> () {
    if (idx == adr_len) {
        return ();
    }

    let universe_idx = idx + UNIVERSE_INDEX_OFFSET;
    ns_lobby_state_functions.universe_addresses_write(universe_idx, adr[idx]);
    ns_lobby_state_functions.universe_address_to_index_write(adr[idx], universe_idx);
...
@external
func gyoza_give_invitation_to_account{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(account: felt) -> () {
    ...
    let (event_counter) = ns_lobby_state_functions.event_counter_read();
    ns_lobby_state_functions.event_counter_increment();
    give_invitation_occurred.emit(event_counter, account);
...
func account_has_ticket_or_invitation{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
...
//
@view
func can_dispatch_player_to_universe{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() -> (curr_head_idx: felt, curr_tail_idx: felt, idle_universe_idx: felt, bool: felt) {
    alloc_locals;
...
func recurse_find_idle_universe{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    idx: felt
) -> (bool_has_idle_universe: felt, idle_universe_idx: felt) {
...
func anyone_dispatch_player_to_universe{
    ...
    //
    let (curr_head_idx, curr_tail_idx, idle_universe_idx, bool) = can_dispatch_player_to_universe();
    with_attr error_message(
            "Unable to dispatch: either not having enough players in queue or not having idle universe available") {
        assert bool = 1;
    }
    ...
    recurse_populate_player_adr_update_queue(arr_player_adr, curr_head_idx, 0);
    ...
    IContractUniverse.activate_universe(
        universe_address, arr_player_adr_len=CIV_SIZE, arr_player_adr=arr_player_adr
    );
    ...
    universe_activation_occurred.emit(
        event_counter, idle_universe_idx, universe_address, CIV_SIZE, arr_player_adr
    );

    return ();
}

func recurse_populate_player_adr_update_queue{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(arr_player_adr: felt*, curr_head_idx: felt, offset: felt) -> () {
    alloc_locals;
    ...
    //
    let (player_adr) = ns_lobby_state_functions.queue_index_to_address_read(
        curr_head_idx + offset + 1
    );
    ...
    // Tail recursion
    //
    recurse_populate_player_adr_update_queue(arr_player_adr, curr_head_idx, offset + 1);
...
func universe_report_play{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    arr_play_len: felt, arr_play: Play*
) -> () {
    alloc_locals;
    ...
    universe_deactivation_occurred.emit(
        event_counter, universe_idx, caller, CIV_SIZE, arr_player_adr
    );
    ...
    IContractDAO.subject_report_play(dao_address, arr_play_len, arr_play);

    return ();
}

func recurse_prepare_arr_player_adr_from_report_play{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(idx: felt, arr_player_adr: felt*, arr_play: Play*) -> () {
    alloc_locals;
    ...
    recurse_prepare_arr_player_adr_from_report_play(idx + 1, arr_player_adr, arr_play);
    return ();
}
