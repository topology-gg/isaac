...
from contracts.design.constants import SCALE_FP, PI, ns_solar_power
...
from contracts.util.vector_ops import (
    distance_2,
    dot_fp,
    magnitude_fp,
    compute_vector_rotate,
    compute_vector_rotate_90,
)
...
struct MacroDistanceSquares {
    distance_sq_to_sun0: felt,
    distance_sq_to_sun1: felt,
    distance_sq_to_sun2: felt,
}

struct MacroVectors {
    vector_plnt_to_sun0: Vec2,
    vector_plnt_to_sun1: Vec2,
    vector_plnt_to_sun2: Vec2,
}
...
struct MacroStatesForTransform {
    macro_distances: MacroDistanceSquares,
    macro_vectors: MacroVectors,
    planet_side_surface_normals: PlanetSideSurfaceNormals,
}

namespace ns_micro_solar {
    func get_macro_distance_squares{range_check_ptr}(macro_state: Dynamics) -> (
    ...
    func get_macro_vectors{range_check_ptr}(macro_state: Dynamics) -> (
        macro_vectors: MacroVectors
    ) {
        return (
            MacroVectors(
    ...
    func get_surface_normals{range_check_ptr}(phi: felt) -> (
        surface_normals: PlanetSideSurfaceNormals
    ) {
        //
    ...
    func get_macro_states_for_transform{range_check_ptr}(macro_state: Dynamics, phi: felt) -> (
    ...
    func get_solar_exposure_fp{range_check_ptr}(
        grid: Vec2, macro_states: MacroStatesForTransform
    ) -> (exposure_fp: felt) {
        alloc_locals;
    ...
    func get_solar_exposure_nonside{range_check_ptr}(macro_states: MacroStatesForTransform) -> (
        exposure: felt
    ) {
        alloc_locals;
        ...
        with_attr error_message(
                "micro_solar.cairo:170 / Pre-division check: about to perform div_fp (ns_solar_power.OBLIQUE_RADIATION, dist_0_sq) but dist_0_sq = 0") {
            assert_not_zero(dist_0_sq);
        }
        let (exposure_0) = div_fp(ns_solar_power.OBLIQUE_RADIATION, dist_0_sq);

        with_attr error_message(
                "micro_solar.cairo:175 / Pre-division check: about to perform div_fp (ns_solar_power.OBLIQUE_RADIATION, dist_1_sq) but dist_1_sq = 0") {
            assert_not_zero(dist_1_sq);
        }
        let (exposure_1) = div_fp(ns_solar_power.OBLIQUE_RADIATION, dist_1_sq);

        with_attr error_message(
                "micro_solar.cairo:180 / Pre-division check: about to perform div_fp (ns_solar_power.OBLIQUE_RADIATION, dist_2_sq) but dist_2_sq = 0") {
    ...
    func get_solar_exposure_side{range_check_ptr}(
        face: felt, macro_states: MacroStatesForTransform
    ) -> (exposure: felt) {
        alloc_locals;
        ...
        if (sign_0 == 1) {
            ...
            with_attr error_message(
                    "micro_solar.cairo:245 / Pre-division check: about to perform div_fp (dot_0, mag_normal) but mag_normal = 0") {
                assert_not_zero(mag_normal);
            }
            let (cos_) = div_fp(dot_0, mag_normal);

            with_attr error_message(
                    "micro_solar.cairo:250 / Pre-division check: about to perform div_fp (cos_, mag_plnt_sun) but mag_plnt_sun = 0") {
            ...
            with_attr error_message(
                    "micro_solar.cairo:259 / Pre-division check: about to perform div_fp (ns_solar_power.BASE_RADIATION, dist_sq) but dist_sq = 0") {
        ...
        if (sign_1 == 1) {
            ...
            with_attr error_message(
                    "micro_solar.cairo:277 / Pre-division check: about to perform div_fp (dot_1, mag_normal) but mag_normal = 0") {
                assert_not_zero(mag_normal);
            }
            let (cos_) = div_fp(dot_1, mag_normal);

            with_attr error_message(
                    "micro_solar.cairo:282 / Pre-division check: about to perform div_fp (cos_, mag_plnt_sun) but mag_plnt_sun = 0") {
                assert_not_zero(mag_plnt_sun);
            }
            let (cos) = div_fp(cos_, mag_plnt_sun);
            ...
            with_attr error_message(
                    "micro_solar.cairo:292 / Pre-division check: about to perform div_fp (ns_solar_power.BASE_RADIATION, dist_sq) but dist_sq = 0") {
                assert_not_zero(dist_sq);
            }
        ...
        if (sign_2 == 1) {
            ...
            with_attr error_message(
                    "micro_solar.cairo:310 / Pre-division check: about to perform div_fp (dot_2, mag_normal) but mag_normal = 0") {
                assert_not_zero(mag_normal);
            }
            let (cos_) = div_fp(dot_2, mag_normal);

            with_attr error_message(
                    "micro_solar.cairo:315 / Pre-division check: about to perform div_fp (cos_, mag_plnt_sun) but mag_plnt_sun = 0") {
                assert_not_zero(mag_plnt_sun);
            }
            let (cos) = div_fp(cos_, mag_plnt_sun);
            ...
            with_attr error_message(
                    "micro_solar.cairo:325 / Pre-division check: about to perform div_fp (ns_solar_power.BASE_RADIATION, dist_sq) but dist_sq = 0") {
                assert_not_zero(dist_sq);
            }
    ...
    func face_is_top_or_bottom{}(face) -> (bool: felt) {
        if (face == 1) {
            return (1,);
        }
