...
from contracts.micro.micro_grids import ns_micro_grids
from contracts.micro.micro_solar import ns_micro_solar, MacroStatesForTransform
...
func resource_update_at_harvester_occurred(
    event_counter: felt, device_id: felt, new_quantity: felt
) {
}

@event
func resource_update_at_transformer_occurred(
    event_counter: felt, device_id: felt, new_quantity_pre: felt, new_quantity_post: felt
) {
}

@event
func resource_update_at_upsf_occurred(
    event_counter: felt, device_id: felt, element_type: felt, new_quantity: felt
) {
}

@event
func energy_update_at_device_occurred(event_counter: felt, device_id: felt, new_quantity: felt) {
}

namespace ns_micro_forwarding {
    func resource_energy_update_at_devices{
        ...
        ) = ns_micro_solar.get_macro_states_for_transform(macro_state_curr, phi_curr);
        ...
        recurse_resource_energy_update_at_devices(
            len=emap_size, idx=0, macro_states=macro_states_for_transform
        );

        return ();
    }

    func recurse_resource_energy_update_at_devices{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(len: felt, idx: felt, macro_states: MacroStatesForTransform) -> () {
        alloc_locals;
        ...
        if (emap_entry.is_deployed == 1) {
            ...
            if (emap_entry.type == ns_device_types.DEVICE_SPG) {
                ...
                //
                let (solar_exposure_fp) = ns_micro_solar.get_solar_exposure_fp(
                    emap_entry.grid, macro_states
                );

                let (
                    energy_generated
                ) = ns_logistics_xpg.spg_solar_exposure_fp_to_energy_generated_per_tick(
                    solar_exposure_fp
                );
                let (curr_energy) = ns_micro_state_functions.device_id_to_energy_balance_read(
                    emap_entry.id
                );
                ...
                energy_update_at_device_occurred.emit(
                    event_counter=event_counter, device_id=emap_entry.id, new_quantity=new_quantity
                );
            ...
            if (emap_entry.type == ns_device_types.DEVICE_NPG) {
                ...
                let (
                    energy_generated
                ) = ns_logistics_xpg.npg_energy_supplied_to_energy_generated_per_tick(curr_energy);
            ...
            if (bool_is_harvester == 1) {
                ...
                let (concentration) = ns_micro_grids.get_resource_concentration_at_grid(
                    emap_entry.grid, element_type
                );
                let (energy_last_tick) = ns_micro_state_functions.device_id_to_energy_balance_read(
                    emap_entry.id
                );
                ...
                let (
                    quantity_curr
                ) = ns_micro_state_functions.harvesters_id_to_resource_balance_read(emap_entry.id);
                ...
                ns_universe_state_functions.event_counter_increment();
                energy_update_at_device_occurred.emit(
                    event_counter=event_counter, device_id=emap_entry.id, new_quantity=0
                );
            ...
            if (bool_is_transformer == 1) {
                ...
                //
                let (
                    element_type_before, element_type_after
                ) = transformer_device_type_to_element_types(emap_entry.type);
                let (balances) = ns_micro_state_functions.transformers_id_to_resource_balances_read(
                    emap_entry.id
                );
                let (energy_last_tick) = ns_micro_state_functions.device_id_to_energy_balance_read(
                    emap_entry.id
                ...
                let (
                    should_transform_quantity
                ) = ns_logistics_transformer.transformer_quantity_per_tick(
                    element_type_before, energy_last_tick
                );
                ...
                let bool = is_le(
                    balances.balance_resource_before_transform, should_transform_quantity
                );
                if (bool == 1) {
    ...
    func resource_transfer_across_utb_sets{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }() -> () {
        //
        // recursively traverse `utx_set_deployed_emap`
        //
        let (emap_size) = ns_micro_state_functions.utx_set_deployed_emap_size_read(
            ns_device_types.DEVICE_UTB
        );
        recurse_resource_transfer_across_utb_sets(len=emap_size, idx=0);
        return ();
    }

    func recurse_resource_transfer_across_utb_sets{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(len, idx) -> () {
        alloc_locals;
        ...
        let (emap_entry) = ns_micro_state_functions.utx_set_deployed_emap_read(
            ns_device_types.DEVICE_UTB, idx
        );
        ...
        if (is_src_tethered * is_dst_tethered == 1) {
            ...
            //
            let (emap_index_src) = ns_micro_state_functions.device_id_to_emap_index_read(
                emap_entry.src_device_id
            );
            let (emap_entry_src) = ns_micro_state_functions.device_emap_read(emap_index_src);
            let (emap_index_dst) = ns_micro_state_functions.device_id_to_emap_index_read(
            ...
            if (bool_src_harvester == 1) {
                ...
                let (src_balance) = ns_micro_state_functions.harvesters_id_to_resource_balance_read(
                    emap_entry.src_device_id
                );
                let (
                    local utb_tether_count
                ) = ns_micro_state_functions.utx_tether_count_of_deployed_device_read(
                    ns_device_types.DEVICE_UTB, emap_entry.src_device_id
                );

                with_attr error_message(
                        "micro_forwarding.cairo:454 / Pre-division check: about to perform unsigned_div_rem (src_balance, utb_tether_count) but utb_tether_count = 0") {
                ...
                //
                let (
                    quantity_should_receive
                ) = ns_logistics_utb.utb_quantity_should_receive_per_tick(
                    quantity_should_send, utb_set_length
                );
                ...
                //
                ns_micro_state_functions.harvesters_id_to_resource_balance_write(
                    emap_entry.src_device_id, src_balance - quantity_should_send
                );
                ...
                let (
                    src_balances
                ) = ns_micro_state_functions.transformers_id_to_resource_balances_read(
                    emap_entry.src_device_id
                );
                ...
                with_attr error_message(
                        "micro_forwarding.cairo:504 / Pre-division check: about to perform unsigned_div_rem (src_balance, utb_tether_count) but utb_tether_count = 0") {
                    assert_not_zero(utb_tether_count);
                }
                let (src_balance_divided, _) = unsigned_div_rem(src_balance, utb_tether_count);
                let (quantity_should_send) = ns_logistics_utb.utb_quantity_should_send_per_tick(
            ...
            if (bool_dst_harvester == 1) {
                ...
                //
                let (max_carry) = harvester_element_type_to_max_carry(element_type);
                let (dst_balance) = ns_micro_state_functions.harvesters_id_to_resource_balance_read(
                    emap_entry.dst_device_id
                );
                ...
                ns_micro_state_functions.harvesters_id_to_resource_balance_write(
                    emap_entry.dst_device_id, new_balance
                );
                ...
                if (bool_dst_opsf == 1) {
                    ...
                    // Note: no max carry limitation
                    //
                    let (dst_balance) = ns_micro_state_functions.opsf_id_to_resource_balances_read(
                        emap_entry.dst_device_id, element_type
                    );
                    ns_micro_state_functions.opsf_id_to_resource_balances_write(
                        emap_entry.dst_device_id, element_type, dst_balance + quantity_received
                    );
    ...
    func energy_transfer_across_utl_sets{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }() -> () {
        //
        // recursively traverse `utx_set_deployed_emap`
        //
        let (emap_size) = ns_micro_state_functions.utx_set_deployed_emap_size_read(
            ns_device_types.DEVICE_UTL
        );
        recurse_energy_transfer_across_utl_sets(len=emap_size, idx=0);
        return ();
    }

    func recurse_energy_transfer_across_utl_sets{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(len, idx) -> () {
        alloc_locals;
        ...
        //
        let (emap_entry) = ns_micro_state_functions.utx_set_deployed_emap_read(
            ns_device_types.DEVICE_UTL, idx
        );
        let is_src_tethered = is_not_zero(emap_entry.src_device_id);
        let is_dst_tethered = is_not_zero(emap_entry.dst_device_id);
        ...
        if (is_src_tethered * is_dst_tethered == 1) {
            ...
            let (src_device_energy) = ns_micro_state_functions.device_id_to_energy_balance_read(
                src_device_id
            );
            let (dst_device_energy) = ns_micro_state_functions.device_id_to_energy_balance_read(
                dst_device_id
            );
            ...
            let (
                ...
                ns_device_types.DEVICE_UTL, src_device_id
            );

            with_attr error_message(
                    "micro_forwarding.cairo:721 / Pre-division check: about to perform unsigned_div_rem (src_device_energy, utl_tether_count) but utl_tether_count = 0") {
                assert_not_zero(utl_tether_count);
            }
            let (src_device_energy_divided, _) = unsigned_div_rem(
                src_device_energy, utl_tether_count
            );
            ...
            let (energy_should_receive) = ns_logistics_utl.utl_energy_should_receive_per_tick(
                energy_should_send, utl_set_length
            );
            ...
            //
            ns_micro_state_functions.device_id_to_energy_balance_write(
                src_device_id, new_src_energy_balance
            );
            ...
            //
            ns_micro_state_functions.device_id_to_energy_balance_write(
                dst_device_id, new_dst_energy_balance
            );
        ...
        recurse_energy_transfer_across_utl_sets(len, idx + 1);
        return ();
    }
