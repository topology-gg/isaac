...
from contracts.design.constants import (
    ...
    assert_device_type_is_nonfungible,
...
func create_new_nonfungible_device_occurred(
    event_counter: felt, owner: felt, type: felt, id: felt
) {
}
...
namespace ns_micro_devices {
    func assert_device_footprint_populable{
    ...
    func assert_valid_unpopulated_and_same_face{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
        ...
        with_attr error_message(
                "device straddling across face {ftgt} and face {face} which is illegal") {
    ...
    func update_grid_with_new_grid_stat{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    ...
    func device_deploy{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        caller: felt, device_id: felt, grid: Vec2
    ) -> () {
        alloc_locals;
    ...
    func recurse_untether_utx_for_deployed_device{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(utx_device_type: felt, id: felt, len: felt, idx: felt) -> () {
        alloc_locals;
        ...
        //
        let (utx_set_label) = ns_micro_state_functions.utx_tether_labels_of_deployed_device_read(
            utx_device_type, id, idx
        );
        ...
        let (
            utx_set_emap_index
        ) = ns_micro_state_functions.utx_set_deployed_label_to_emap_index_read(
            utx_device_type, utx_set_label
        );
        let (utx_set_emap_entry) = ns_micro_state_functions.utx_set_deployed_emap_read(
    ...
    func device_pickup_by_grid{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        // Update `device_emap`
        //
        let (emap_index) = ns_micro_state_functions.device_id_to_emap_index_read(
        ...
        //
        let (utb_tether_count) = ns_micro_state_functions.utx_tether_count_of_deployed_device_read(
            ns_device_types.DEVICE_UTB, grid_stat.deployed_device_id
        );
        ...
        let (utl_tether_count) = ns_micro_state_functions.utx_tether_count_of_deployed_device_read(
            ns_device_types.DEVICE_UTL, grid_stat.deployed_device_id
        );
        recurse_untether_utx_for_deployed_device(
        ...
        update_grid_stat:
        update_grid_with_new_grid_stat(
            emap_entry.type,
            emap_entry.grid,
            GridStat(
            populated=0,
    ...
    func upsf_build_fungible_device{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(caller: felt, grid: Vec2, device_type: felt, device_count: felt) -> () {
        alloc_locals;
        ...
        //
        let (
            energy: felt, resource_arr_len: felt, resource_arr: felt*
        ) = ns_manufacturing.get_resource_energy_requirement_given_device_type(device_type);
        local energy_should_consume = energy * device_count;

        //
        // Consume upsf energy; revert if insufficient
        //
        let (local curr_energy) = ns_micro_state_functions.device_id_to_energy_balance_read(
            opsf_device_id
        );
        with_attr error_message(
                "insufficient energy; {energy_should_consume} required, {curr_energy} available at UPSF.") {
            assert_le(energy_should_consume, curr_energy);
        }
        ns_micro_state_functions.device_id_to_energy_balance_write(
            opsf_device_id, curr_energy - energy_should_consume
        );
        ...
        //
        let (curr_amount) = ns_micro_state_functions.fungible_device_undeployed_ledger_read(
            caller, device_type
        );
        ns_micro_state_functions.fungible_device_undeployed_ledger_write(
            caller, device_type, curr_amount + device_count
        );

        return ();
    }

    func upsf_build_nonfungible_device{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(caller: felt, grid: Vec2, device_type: felt, device_count: felt) -> () {
        alloc_locals;
    ...
    func recurse_create_new_undeployed_device{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(idx: felt, len: felt, block_height: felt, owner: felt, device_type: felt) -> () {
        if (idx == len) {
            return ();
        }
        ...
        // Tail recursion
        //
        recurse_create_new_undeployed_device(idx + 1, len, block_height, owner, device_type);

        return ();
    }

    func create_new_nonfungible_device{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(block_height: felt, owner: felt, device_type: felt) -> () {
        alloc_locals;
        ...
        create_new_nonfungible_device_occurred.emit(
            event_counter, owner, device_type, new_device_id
        );

        return ();
    }

    func recurse_consume_device_balance_at_opsf{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(opsf_device_id: felt, device_count: felt, len: felt, arr: felt*, idx: felt) -> () {
        alloc_locals;
        ...
        with_attr error_message(
                "insufficient quantity of type {element_type}; {quantity_should_consume} required, {curr_balance} available at UPSF.") {
        ...
        ns_micro_state_functions.opsf_id_to_resource_balances_write(
            opsf_device_id, idx, curr_balance - quantity_should_consume
        );

        //
        // Tail recursion
        //
        recurse_consume_device_balance_at_opsf(opsf_device_id, device_count, len, arr, idx + 1);
        return ();
    }
    ...
    func launch_all_deployed_ndpe{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (impulse_sum_in_micro_coord: Vec2) = recurse_operate_all_ndpes(
            len=emap_size, idx=0, impulse_sum=Vec2(0, 0), civ_idx=civ_idx
        );
        ...
        let (impulse_sum_in_macro_coord: Vec2) = compute_vector_rotate(
            vec=impulse_sum_in_micro_coord, phi=curr_phi
        );
        ...
        ns_macro_state_functions.impulse_cache_write(
            Vec2(
    ...
    func recurse_operate_all_ndpes{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        len: felt, idx: felt, impulse_sum: Vec2, civ_idx: felt
        ...
        if (bool_is_ndpe * emap_entry.is_deployed == 1) {
            ...
            //
            let (grid_stat: GridStat) = ns_micro_state_functions.grid_stats_read(
                civ_idx, emap_entry.grid
            );
            ns_universe_state_functions.civilization_player_address_to_has_launched_ndpe_write(
                grid_stat.deployed_device_owner, 1
            );
        ...
        let (impulse_sum_final: Vec2) = recurse_operate_all_ndpes(
            len, idx + 1, Vec2(impulse_sum.x + impulse.x, impulse_sum.y + impulse.y), civ_idx
        );
    ...
    func are_producer_consumer_relationship{range_check_ptr}(
        ...
        with_attr error_message(
                "resource producer-consumer relationship check failed, with utx_device_type = {z}, device_type0 = {x} and device_type1 = {y}") {
            assert 1 = 0;
        }
        return ();
    }
