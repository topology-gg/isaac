...
from contracts.design.constants import ns_device_types, CIV_SIZE
from contracts.micro.micro_state import ns_micro_state_functions
...
namespace ns_micro_reset {
    ...
    func recurse_over_address_reset_fungible_device_undeployed_ledger{
        ...
        //
        let (player_address) = ns_universe_state_functions.civilization_player_idx_to_address_read(
            idx
        );
        recurse_over_device_type_given_address_reset_fungible_device_undeployed_ledger(
            player_address, ns_device_types.DEVICE_UTB
        );

        //
        // Tail recursion
        //
        recurse_over_address_reset_fungible_device_undeployed_ledger(idx + 1);
        return ();
    }

    func recurse_over_device_type_given_address_reset_fungible_device_undeployed_ledger{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(player_address: felt, device_type: felt) -> () {
        alloc_locals;
