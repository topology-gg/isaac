...
namespace ns_micro_utx {
    ...
    func utx_deploy{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        //
        let (
            local owned_utx_amount
        ) = ns_micro_state_functions.fungible_device_undeployed_ledger_read(
            caller, utx_device_type
        );
        local len = locs_len;
        with_attr error_message(
                "attempt to deploy {len} amount of UTXs but owning only {owned_utx_amount}") {
        ...
        ns_micro_devices.are_producer_consumer_relationship(
            utx_device_type, src_grid_stat.deployed_device_type, dst_grid_stat.deployed_device_type
        );
        ...
        ns_micro_state_functions.fungible_device_undeployed_ledger_write(
            caller, utx_device_type, owned_utx_amount - locs_len
        );
        ...
        //
        ns_micro_state_functions.utx_set_deployed_label_to_emap_index_write(
            utx_device_type, new_label, emap_size
        );

        //
        // For src and dst device, update their `utx_tether_count_of_deployed_device` and `utx_tether_labels_of_deployed_device`
        //
        let (count) = ns_micro_state_functions.utx_tether_count_of_deployed_device_read(
            utx_device_type, src_emap_entry.id
        );
        ns_micro_state_functions.utx_tether_count_of_deployed_device_write(
            utx_device_type, src_emap_entry.id, count + 1
        );
        ns_micro_state_functions.utx_tether_labels_of_deployed_device_write(
            utx_device_type, src_emap_entry.id, count, new_label
        );

        let (count) = ns_micro_state_functions.utx_tether_count_of_deployed_device_read(
            utx_device_type, dst_emap_entry.id
        );
        ns_micro_state_functions.utx_tether_count_of_deployed_device_write(
            utx_device_type, dst_emap_entry.id, count + 1
        );
        ns_micro_state_functions.utx_tether_labels_of_deployed_device_write(
            utx_device_type, dst_emap_entry.id, count, new_label
        );
    ...
    func recurse_utx_deploy{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        //
        ns_micro_state_functions.utx_deployed_index_to_grid_write(
            utx_device_type, utx_idx, arr[idx]
        );
        ...
        recurse_utx_deploy(
            caller, utx_device_type, len, arr, idx + 1, utx_idx + 1, set_label, civ_idx
        );
        return ();
    }
    ...
    func utx_pickup_by_grid{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        //
        let (emap_size_curr) = ns_micro_state_functions.utx_set_deployed_emap_size_read(
            utx_device_type
        );
        let (emap_index) = ns_micro_state_functions.utx_set_deployed_label_to_emap_index_read(
            utx_device_type, utx_set_deployed_label
        );
        let (emap_entry) = ns_micro_state_functions.utx_set_deployed_emap_read(
            utx_device_type, emap_index
        );
        ...
        //
        let (amount_curr) = ns_micro_state_functions.fungible_device_undeployed_ledger_read(
            caller, utx_device_type
        );
        ns_micro_state_functions.fungible_device_undeployed_ledger_write(
            caller, utx_device_type, amount_curr + utx_end_index - utx_start_index
        );
        ...
        //
        let (emap_entry_last) = ns_micro_state_functions.utx_set_deployed_emap_read(
            utx_device_type, emap_size_curr - 1
        );
        ns_micro_state_functions.utx_set_deployed_emap_write(
            utx_device_type, emap_index, emap_entry_last
        );
        ns_micro_state_functions.utx_set_deployed_emap_write(
            utx_device_type, emap_size_curr - 1, UtxSetDeployedEmapEntry(0, 0, 0, 0, 0)
        );
        ns_micro_state_functions.utx_set_deployed_emap_size_write(
            utx_device_type, emap_size_curr - 1
        );
        ...
        //
        let (tether_count) = ns_micro_state_functions.utx_tether_count_of_deployed_device_read(
            utx_device_type, emap_entry.src_device_id
        );
        ns_micro_state_functions.utx_tether_count_of_deployed_device_write(
            utx_device_type, emap_entry.src_device_id, tether_count - 1
        );
        ns_micro_state_functions.utx_tether_labels_of_deployed_device_write(
            utx_device_type, emap_entry.src_device_id, tether_count - 1, 0
        );

        let (tether_count) = ns_micro_state_functions.utx_tether_count_of_deployed_device_read(
            utx_device_type, emap_entry.dst_device_id
        );
        ns_micro_state_functions.utx_tether_count_of_deployed_device_write(
            utx_device_type, emap_entry.dst_device_id, tether_count - 1
        );
        ns_micro_state_functions.utx_tether_labels_of_deployed_device_write(
            utx_device_type, emap_entry.dst_device_id, tether_count - 1, 0
        );

        return ();
    }

    func recurse_pickup_utx_given_start_end_utx_index{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(utx_device_type: felt, start_idx: felt, end_idx: felt, idx: felt, civ_idx: felt) -> () {
        alloc_locals;
        ...
        let (grid_to_clear) = ns_micro_state_functions.utx_deployed_index_to_grid_read(
            utx_device_type, start_idx + idx
        );
        ns_micro_state_functions.grid_stats_write(civ_idx, grid_to_clear, GridStat(0, 0, 0, 0));

        recurse_pickup_utx_given_start_end_utx_index(
            utx_device_type, start_idx, end_idx, idx + 1, civ_idx
        );
        return ();
    }
