...
async def test_micro (account_factory):
    ...
    ret = await contract.grid_stats_read( contract.Vec2(user1_harvester_grid[0], user1_harvester_grid[1]) ).call()
    ...
    ret = await contract.grid_stats_read( contract.Vec2(user1_refinery_grid[0], user1_refinery_grid[1]) ).call()
    ...
    ret = await contract.grid_stats_read( contract.Vec2(user2_harvester_grid[0], user2_harvester_grid[1]) ).call()
    ...
    ret = await contract.grid_stats_read( contract.Vec2(user2_refinery_grid[0], user2_refinery_grid[1]) ).call()
    ...
    for x,y in zip ([151, 152, 153, 154], [150, 150, 150, 150]):
        ret = await contract.grid_stats_read(
            contract.Vec2(x,y)
        ).call()
        assert ret.result.grid_stat.populated == 1
    ...
    for i in range(2):
        ...
        ret = await contract.utx_tether_count_of_deployed_device_read(12, device_id).call()
        assert ret.result.count == 1

        ## check the label of tethered utb meets expected
        ret = await contract.utx_tether_labels_of_deployed_device_read(12, device_id, 0).call()
    ...
    for (x,y) in [user1_harvester_grid, user1_refinery_grid, user2_harvester_grid, user2_refinery_grid,user2_refinery_tether_grid] + [(151,150), (152,150), (153,150), (154,150)] + [(99,101), (100,101), (101,101), (101,100), (101,99)]:
        with pytest.raises(Exception) as e_info:
            await users[3]['signer'].send_transaction(
                ...
                selector_name = 'flat_device_pickup_by_grid',
                calldata=[
                    x, y # grid
                ])
    ...
    ret = await contract.grid_stats_read( contract.Vec2(user3_harvester_grid[0], user3_harvester_grid[1]) ).call()
    ...
    ret = await contract.grid_stats_read( contract.Vec2(user3_refinery_grid[0], user3_refinery_grid[1]) ).call()
    ...
    ret = await contract.device_deployed_emap_read(4).call()
    ...
    ret = await contract.device_deployed_emap_read(5).call()
    ...
    # # Note: commented out because this test was implemented when resource & energy updates were not parametrized;
    # #       after implementing parametrization, a model for resource & energy management is required for tests moving forward
    # #
    # # LOGGER.info (f'> ------------')
    ...
    ret = await contract.device_undeployed_ledger_read(users[1]['account'].contract_address, 7).call()
    ...
    for i in range(5):
        ...
        ret = await contract.grid_stats_read(ret.result.emap_entry.grid).call()
    ...
    ret = await contract.utx_set_deployed_emap_read(12, 0).call()
    ...
    await users[2]['signer'].send_transaction(
        account = users[2]['account'], to = contract.contract_address,
        selector_name = 'flat_utx_pickup_by_grid',
        calldata=[
            101, 101
        ])

    ret = await contract.device_undeployed_ledger_read(users[2]['account'].contract_address, 12).call()
    ...
    for (x,y) in zip([99, 100, 101, 101, 101],  [101, 101, 101, 100, 99]):
        ret = await contract.grid_stats_read(contract.Vec2(x,y)).call()
    ...
    ret = await contract.utx_tether_count_of_deployed_device_read(12, user2_harvester_id).call()
    assert ret.result.count == 0
    ret = await contract.utx_tether_count_of_deployed_device_read(12, user2_refinery_id).call()
    assert ret.result.count == 0

    ret = await contract.utx_tether_labels_of_deployed_device_read(12, user2_harvester_id, 0).call()
    assert ret.result.utx_set_label == 0
    ret = await contract.utx_tether_labels_of_deployed_device_read(12, user2_refinery_id, 0).call()
    assert ret.result.utx_set_label == 0
