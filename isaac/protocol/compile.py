...
def _compile_contract (path, name):
	cmd = f'starknet-compile {path} --output ../deploy/artifacts/isaac/{name}_compiled.json --abi ../deploy/artifacts/isaac/{name}_abi.json'
	cmd = cmd.split(' ')
...
name = 'charter'
path = 'contracts/charter.cairo'
_compile_contract (path, name)

name = 'fsm'
path = 'contracts/fsm.cairo'
_compile_contract (path, name)

name = 'dao'
path = 'contracts/dao.cairo'
_compile_contract (path, name)
